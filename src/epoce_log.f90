...
!             endif
    ...
    do t=1,nt ! boucle sur les temps de validation
        ...
        call derivc_condT_logN(b,np,J_cond,rlindiv,nobs,nsujet,indT,valT(t))
...
!                        derivc_condt
!------------------------------------------------------------
    subroutine derivc_condt_LogN(b,m,V,rlindiv,nobs,nsujet,indT,valT)
    ...
    do i=1,nsujet
        ...
        if (indT(i).eq.1) then ! contribution de i sachant T
            rlindiv(i) = funcpi_LogN(nobs,b,m,id,z,id,z,i,1,valT)
!             write(*,*)nsujet,rlindiv(i)
        ...
        do k=1,m
            ...
            if (indT(i).eq.1) then ! calcul des derivees sachant T
                temp1 = funcpi_LogN(nobs,b,m,k,th,id,z,i,1,valT)
                temp2 = funcpi_LogN(nobs,b,m,k,thn,id,z,i,1,valT)
            ...
            ! calcul des derivees
            temp1 = funcpi_LogN(nobs,b,m,k,th,id,z,i,2,valT)
            temp2 = funcpi_LogN(nobs,b,m,k,thn,id,z,i,2,valT)
            if (temp1.eq.-1.d9.or.temp2.eq.-1.d9) then 
                V = 0.d0
                !rlindiv = -1.d9
                goto 777
...
777 continue
    ...
    end subroutine derivc_condt_LogN
...
!------------------------------------------------------------
    ...
    if (choix.eq.1) then
        integrale1 = 0.d0
        call gauherEpoce(integrale1,1,bh,np,i,nobs,valT,0)
        integrale2 = 0.d0
        call gauherEpoce(integrale2,2,bh,np,i,nobs,valT,0)

        funcpi_LogN = integrale1/integrale2 ! conditionnellement aux reccurences
    else
        integrale1 = 0.d0
        call gauherEpoce(integrale1,1,bh,np,i,nobs,valT,1)

        funcpi_LogN = integrale1
    endif

    funcpi_LogN = dlog(funcpi_LogN)

    if ((funcpi_LogN.ne.funcpi_LogN).or.(abs(funcpi_LogN).gt.1.d30)) then
        funcpi_LogN = -1.d9
    end if

    return

    end function funcpi_LogN

!------------------------------------------------------------
!------------------------------------------------------------

    subroutine gauherEpoce(ss,choix,bh,np,i,nobs,valT,all)
    ...
    double precision ::auxfunca,func1E_LogN,func2E_LogN
    external::func1E_LogN,func2E_LogN
    integer::j
    ...
    ss = 0.d0
     if (typeof.eq.0) then
        do j=1,20
            if (choix.eq.1) then
            auxfunca = func1E_LogN(x2(j),bh,np,i,nobs,valT,all)
            ss = ss+w2(j)*(auxfunca)
        else
            if (choix.eq.2) then 
                auxfunca = func2E_LogN(x2(j),bh,np,i,nobs,valT)
                ss = ss+w2(j)*(auxfunca)
            endif
        endif
    end do
            ...
            if (choix.eq.2) then 
                auxfunca = func2E_LogN(x3(j),bh,np,i,nobs,valT)
                ss = ss+w3(j)*(auxfunca)
            endif
        endif
    end do
...
! ------------ Reccurent ------------- !
    ...
    do k=1,nobs ! toutes les recurrences pour l'individu i
        if (g(k).eq.i) then
            ...
            func1E_LogN = func1E_LogN * (sut1(1)/sut0(1))**(dexp(frail)*vet)

            if ((func1E_LogN.ne.func1E_LogN).or.(abs(func1E_LogN).gt.1.d30)) then
                func1E_LogN = -111.d9
           !      print*,"1"
                goto 1000
            end if
            ...
            if (c(k).eq.1) then
                ...
                func1E_LogN = func1E_LogN * dexp(frail)*lam*vet
...
! ------------ Death ------------- !
    ...
    func1E_LogN = func1E_LogN * sudc(2)**(dexp(frail*alpha) * vet2)
    ...
    if (cdc(i).eq.1) then
        ...
        func1E_LogN = func1E_LogN * dexp(frail*alpha) * lamdc*vet2
...
! ------------ Reccurent ------------- !
    ...
    do k=1,nobs ! toutes les recurrences pour l'individu i
        if ((g(k).eq.i).and.(t1(k).le.valT)) then
            ...
            func2E_LogN = func2E_LogN * (sut1(1)/sut0(1))**(dexp(frail)*vet)

            if ((func2E_LogN.ne.func2E_LogN).or.(abs(func2E_LogN).gt.1.d30)) then
         !        print*,"1",func2E
                func2E_LogN = -11.d9
                goto 1000
            end if
            ...
            if (c(k).eq.1) then
                ...
                func2E_LogN = func2E_LogN * lam*vet*dexp(frail)
...
! ------------ Death ------------- !
    ...
    func2E_LogN = func2E_LogN * sudc(2)**(dexp(frail*alpha )* vet2)
