...
! la prediction des effets aleatoires n'est pas la meme pour gamma ou log-normal
    ...
    !=============================================================================
!                       CALCUL DES RESIDUS de MARTINGALES Shared correlated
!=============================================================================
    subroutine ResidusMartingale_corr(b,np,namesfuncres,Resmartingale, frailtypred)
    ...
    integer:: i,k
    double precision,dimension(np),intent(in)::b
    double precision,dimension(nsujet),intent(out)::Resmartingale
    double precision,dimension(nsujet),intent(out)::frailtypred
    ...
    k = 0
        ...
        allocate(vuuu(nmemb(indg)),vres((nmemb(indg))*((nmemb(indg))+3)/2))
         vuuu=0.9d0
            ind_cur = k
            call marq98res(vuuu,nmemb(indg),nires,vres,rlres,ierres,istopres,cares,cbres,ddres,namesfuncres)

            if (istopres.eq.1) then
            do i=1,nmemb(indg)
                Resmartingale(k+i)=c(k+i)-(dexp(vuuu(i)))*cumulhaz_corr(k+i)
                frailtypred(k+i) = vuuu(i)
            end do
            else
                ! non convergence ou erreur de calcul de la fonction a maximiser
                do i=1,nmemb(indg)
                Resmartingale(i+k) = 0.d0
                frailtypred(i+k) = 0.d0
                end do
            endif
            k = k+ nmemb(indg)
            deallocate(vuuu,vres)
        end do


    end subroutine ResidusMartingale_corr
...
!=============================================================================
    ...
    do indg=1,nfam
        ...
        if (istopres.eq.1) then
            do i=1,fsize(indg)
                if(Nrec(indiv).gt.0) then
                Resmartingale(indiv) =1/Nrec(indiv) -(((vuuu(1)*vuuu(1))**xi)*&
                        (vuuu(1+i)*vuuu(1+i)))*cumulhaz1(indg,i)/Nrec(indiv)
                else 
                Resmartingale(indiv) =0.d0 -(((vuuu(1)*vuuu(1))**xi)*&
