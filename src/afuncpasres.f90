...
!!!!
!!!! Calcul Residus shared log-normal correlated
!!!!
    double precision function funcpasres_corr(uu,np,id,thi,jd,thj)
    ...
    integer,intent(in)::id,jd,np
    integer:: ier,j,jj,k
    double precision:: eps,quad
    double precision,dimension(np)::bh
    double precision,dimension(np),intent(in)::uu
    double precision,intent(in)::thi,thj
    double precision,dimension(nmemb(indg))::ui
    double precision,dimension(:,:),allocatable::Ut_inv
    ...
    allocate(Ut(nmemb(indg),nmemb(indg)),Ut_inv(nmemb(indg),nmemb(indg)),Ut_table(nmemb(indg)*(nmemb(indg)+1)/2))
    Ut = 0.d0
    do j=1,nmemb(indg)
        do k=j,nmemb(indg)
            if(corrRE.eq.2) Ut(j,k) = sig2*Kmat1(ind_cur+j,ind_cur+k) + sig22*Kmat2(ind_cur+j,ind_cur+k)
            if(corrRE.eq.1.or.corrRE.eq.3) Ut(j,k) = sig2*Kmat1(ind_cur+j,ind_cur+k)
            Ut(k,j) = Ut(j,k)
            jj = j + k*(k-1)/2
            Ut_table(jj) = Ut(j,k)
        end do
    ...
    eps = 1.d-7
    call dsinvj(Ut_table, nmemb(indg), eps, ier)
    if (ier.eq.-1) then
        funcpasres_corr = -1.d9
        deallocate(Ut, Ut_inv, Ut_table)
        return
    end if
    ...
    do j=1,nmemb(indg)
        do k=j,nmemb(indg)
            jj = j + k*(k-1)/2
            Ut_inv(j,k) = Ut_table(jj)
            Ut_inv(k,j) = Ut_table(jj)
        end do
    end do

    ui = bh(1:nmemb(indg))
    quad = dot_product(ui, matmul(Ut_inv, ui))

    funcpasres_corr = 0.d0
    do j = 1,nmemb(indg)
        funcpasres_corr = funcpasres_corr + c(ind_cur+j)*ui(j) - dexp(ui(j))*cumulhaz_corr(ind_cur+j)
    end do
    funcpasres_corr = dexp(funcpasres_corr - quad/2.d0)

     deallocate(Ut, Ut_inv,Ut_table)
    return

    end function funcpasres_corr
...
!!!!
    ...
    !    prod5 = prod5 * frail2(j)**(Nrec_ind(sum(fsize(1:(indg-1)))+j)+alpha*cdc(sum(fsize(1:(indg-1)))+j))
        ...
        prod1 = prod1 * (frail2(j)**(Nrec_ind(sum(fsize(1:(indg-1)))+j)+alpha*cdc(sum(fsize(1:(indg-1)))+j)))&
                        * dexp(-frail1**xi * frail2(j) * cumulhaz1(indg,j))
    ...
    res = prod1 * prod3 * frail1**((1.d0/eta) - 1.d0) * dexp(-frail1/eta) * prod2 * &! frail1**(1.d0/eta - 1.d0) *
            prod4 *frail1**(Nrec_fam(indg)*xi+Ndc_fam(indg))
...
!!!! =============================== Calcul Residus joint bivariate - longitudinales et deces ==============================
          ...
          matv = 0.d0
            ...
            if(s_cag_id.eq.1)then
                    do k = 0,nmesy(indg)-1
                        ...
                        !            if(indg.eq.14)write(*,*)(1.d0-alnorm((mu1_res(k+1)-s_cag)/sqrt(sigmae),upper)),mu1_res(k+1)
                            else
  ...
  !  if(indg.eq.14)write(*,*)yscalar,prod_cag,res  
        if ((res.ne.res).or.(abs(res).ge. 1.d30)) then
            funcpajres_biv=-1.d9
            goto 222
        end if
