...
!========================          FUNCPA_WEIB          ====================
    ...
    etaT,betaT,currentage,proband,agemin
    use residusM
...
! *** NOUVELLLE DECLARATION F90 :
    ...
    double precision::thi,thj,dnb,sum,inv,res,vet,slogasc
    double precision,dimension(np)::b,bh
    double precision,dimension(ngmax)::res1,res2,res3,res6
    ...
    do i = 1,nsujet 
        if(proband(i).eq.1) then 
            if(t1(i).lt.currentage(i)) then
                if(1-dexp(-res6(i)).ne.0)slogasc = slogasc + dlog(1-dexp(-res6(i)))
            else 
