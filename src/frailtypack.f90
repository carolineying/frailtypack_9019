! CFM - correlated frailty model
    ...
    timedep0,nbinnerknots0,qorder0,filtretps0,BetaTpsMat,EPS, &
    Kmat10, Kmat20,corrRE_proband0, currentage0,&
    agemin_sig_fixed,Fisher0,IOut)
...
! Obs: noVar=0 indicates no variables but I need to pass all 0's in vaxAux
    ...
    double precision,dimension(4)::agemin_sig_fixed !CFM and cubature controls
    ...
    double precision,dimension(np+1)::Fisher0
    integer, dimension(nsujetAux+3)::corrRE_proband0        !CFM and cubature controls
    ! The first nsujetAux entries are the public current-age vector.  The
    ! remaining entries are an internal adaptive-integration control/output
    ! buffer.  Packing this buffer here preserves frailpenal's historical
    ! 65-argument .Fortran ABI (R rejects calls with more arguments).
    double precision,dimension(nsujetAux+8+ngAux*(adaptive_nidiag+adaptive_nddiag))::currentage0
    !******************************************   Add JRG January 05
    ...
    double precision,dimension(np,np)::H_hessOut,HIHOut,IOut
...
!AD:add
   ...
   ! double precision,external::funcpassplines_fafm,funcpasweib_fam
...
!predictor
    double precision,dimension(nsujetAux),intent(out)::Resmartingale,frailtypred,frailtysd,frailtyvar
    double precision,external::funcpasres, funcpasres_corr
    ...
    integer,dimension(6)::adaptive_i0
    double precision,dimension(2)::adaptive_d0
    integer::adaptive_buffer_offset
    ...
    double precision,dimension(nsujetAux,nsujetAux):: Kmat10, Kmat20 !CFM
!verification
    ...
    !CFM 
    allocate(Kmat1(nsujetAux,nsujetAux),Kmat2(nsujetAux,nsujetAux),currentage(nsujetAux),proband(nsujetAux))
    do i=1,nsujetAux
        currentage(i) = currentage0(i)
        proband(i) = corrRE_proband0(i+1)
    do j=1,nsujetAux
    Kmat1(i,j) = Kmat10(i,j)
    Kmat2(i,j) = Kmat20(i,j)
    end do
    end do
    corrRE = corrRE_proband0(1)

    agemin = agemin_sig_fixed(1)
    sig_fixed = agemin_sig_fixed(2)

    cubature_minpts = corrRE_proband0(nsujetAux+2)
    cubature_maxpts = corrRE_proband0(nsujetAux+3)
    cubature_epsabs = agemin_sig_fixed(3)
    cubature_epsrel = agemin_sig_fixed(4)
    cubature_calls = 0
    cubature_failures = 0
    cubature_max_neval = 0
    cubature_max_dimension = 0
    cubature_max_abserr = 0.d0
    cubature_max_relerr = 0.d0
    cubature_last_rule = 0
    cubature_signed_sum_status = 0
    cubature_cap_blocked = 0
    cubature_log_positive = -huge(1.d0)
    cubature_log_negative = -huge(1.d0)
    cubature_cancellation_ratio = 0.d0
    do i=1,6
        adaptive_i0(i) = int(currentage0(nsujetAux+i))
    end do
    adaptive_d0(1:2) = currentage0(nsujetAux+7:nsujetAux+8)
    call initialize_posterior_adaptive(nsujetAux,ngAux,adaptive_i0,adaptive_d0)
    
    if(Fisher0(1) == 1.d0) then 
        estimate  = 0
    else 
        estimate = 1
    end if
...
!     scaleweib = 0.d0
    ...
    if (typeof == 0) then
        allocate(nt0(nsujetmax),nt1(nsujetmax),nt2(nsujetmax),ntU(nsujetmax))
        nt0=0
        nt1=0
        nt2=0 !CFM
        ntU=0
...
!Al: utile pour le calcul d'integrale avec distribution log normale
   allocate(res3(ngmax),res5(nsujetmax),res1(ngmax),sig_prob(nsujetmax),sig_prob2(nsujetmax),&
    res6(nsujetmax),res2_ind(nsujetmax),res3_ind(nsujetmax))
!Al

   allocate(nig(ngmax),nmemb(ngmax))
    nig=0
    nmemb = 0 !CFM
    allocate(Residus(ngmax),cumulhaz(ngmax),cumulhaz_corr(nsujetmax),vuu(1))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    j=0
    nbou2=0
    id=1    
...
! attention on a un nombre de groupes, mais le numero de groupe peut depasser la liste
    ...
    nmemb(groupe) = nmemb(groupe)+1 !CFM
    ...
    do i = 1,(2*nsujet+sum(icAux)) ! changement comme indique plus haut
        do k = 1,nsujet
            if (t0(k).ge.min) then
               ...
               if ((t1(k)-agemin).ge.min) then
                if((t1(k)-agemin).lt.max)then
                    max = t1(k)-agemin
                endif
            endif
...
!Al:
            do j=1,ndate
                ...
                if(date(j).eq.t0(i))then
                 ...
                 if(date(j).eq.(t1(i)-agemin))then
                  nt1(i)=j
                endif

                if(date(j).eq.(currentage(i)-agemin))then
                    nt2(i)=j
                endif
...
!------ initialisation des parametres
    ...
    b(1) = sqrt(3.d0) !2.6d0!3.d0!3.d0 
    b(2) = sqrt(1/0.007d0 )! 6.8d0! 10.d0!10.d0
    b(3) = 0.26d0!0.1d0 !0.2d0!225d0 
...
!---------------------Calcul residus de martingales
    ...
    if(typeof==0) then
        ...
        if((istopp(1) == 0).and.(istopp(2) == 0)) then
            ...
            allocate(vecuiRes(ng),I_hess(1,1),H_hess(1,1),post_esp(ng),post_SD(ng))
            if (corrRE.eq.0) allocate(vres((1*(1+3)/2)))
            effetres = effet

            if (effet.eq.1.and.corrRE.eq.0) then
                Call ResidusMartingale(b,np,funcpasres,Resmartingale,frailtypred,frailtyvar,frailtysd)
            else if (corrRE.ge.1) then
                Call ResidusMartingale_corr(b,np,funcpasres_corr,Resmartingale, frailtypred)
                frailtyvar=0.d0
                frailtysd=0.d0
            else
                Resmartingale=0.d0
            ...
            do i=1,nsujet
                if (effet == 1) then
                    if (corrRE.ge.1) then
                        linearpred(i)=Xbeta(1,i)+frailtypred(i)
                    else if (logNormal.eq.0) then
            ...
            deallocate(I_hess,H_hess,vecuiRes,post_esp,post_SD)
        else
            !deallocate(I_hess,H_hess)
            Resmartingale=0.d0
            frailtypred=0.d0
...
!     endif
    ...
    deallocate(t0,t1,tU,c,d,stra,g,nig,nmemb,ve,Hspl_hess,hess, &
    mm3,mm2,mm1,mm,im3,im2,im1,im,date,cumulhaz,cumulhaz_corr,Residus,vuu, &
    RisqCumul,res5,res3,res1,res6,sig_prob,sig_prob2,res2_ind,res3_ind,&
    filtretps,knotsTPS,innerknots,betatps,betatps3)

    if (typeof == 0) then
        deallocate(nt0,nt1,nt2,ntU,zi)
    ...
    Kmat10(1,1) = dble(cubature_calls)
    Kmat10(2,1) = dble(cubature_failures)
    Kmat10(1,2) = dble(cubature_max_neval)
    Kmat10(2,2) = dble(cubature_max_dimension)
    Kmat20(1,1) = cubature_max_abserr
    Kmat20(2,1) = cubature_max_relerr
    adaptive_buffer_offset = nsujetAux + 8
    do j=1,adaptive_nidiag
        do i=1,ngAux
            adaptive_buffer_offset = adaptive_buffer_offset + 1
            currentage0(adaptive_buffer_offset) = dble(adaptive_diag_i(i,j))
        end do
    end do
    do j=1,adaptive_nddiag
        do i=1,ngAux
            adaptive_buffer_offset = adaptive_buffer_offset + 1
            currentage0(adaptive_buffer_offset) = adaptive_diag_d(i,j)
        end do
    end do
    call finalize_posterior_adaptive()
deallocate(Kmat1, Kmat2,currentage,proband)
...
! func est l integrant, ss le resultat de l integrale sur -infty , +infty
    ...
    SUBROUTINE gauherASC2(ss,choix,sig1)
    ...
    double precision,intent(out)::ss
   double precision,intent(in)::sig1
   integer,intent(in)::choix
   
    double precision::auxfunca,func2ASC
    external::func2ASC,gauherASC
    integer::j

    ss=0.d0
    do j=1,20
        frailpol = x2(j)
        call gauherASC(auxfunca,choix, sig1)
        ss = ss+w2(j)*(auxfunca)
    end do

    return

    END SUBROUTINE gauherASC2


   
    
    
       SUBROUTINE gauherASC(ss,choix,sig)
...
use donnees_indiv,only : frailpol
   ...
   integer,intent(in)::choix
    ...
    double precision::auxfunca,func1ASC,func2ASC
    external::func1ASC,func2ASC
    integer::j

    ss=0.d0
    do j=1,20
        if(choix.eq.1)auxfunca=func1ASC(x2(j),sig)
        if(choix.eq.2)auxfunca=func2ASC(frailpol,x2(j),sig)
        ss = ss+w2(j)*(auxfunca)
    end do

    return

    END SUBROUTINE gauherASC


    double precision function func1ASC(frail,sig)

    use comon,only:auxig,sig_prob,res6
    
    IMPLICIT NONE

    double precision,intent(in)::frail,sig
    double precision,parameter::pi=3.141592653589793d0


  !  func1ASC = dexp(-res6(auxig)*dexp(frail)-(frail**2.d0)/(2.d0*sig))/sqrt(2.d0*pi*sig)
 func1ASC = dexp(-res6(auxig)*dexp(sqrt(sig)*frail)-(frail**2.d0)/(2.d0))/sqrt(2.d0*pi)!*sig)
 !write(*,*)auxig,func1ASC,-res6(auxig)*dexp(sqrt(sig)*frail),-(frail**2.d0)/(2.d0),frail
!write(*,*)func1ASC,dexp(-res6(auxig)*dexp(sqrt(sig)*frail)-(frail**2.d0)/(2.d0))
    return

    end function func1ASC
    
    
        double precision function func2ASC(frail,frail2,sig1)

    use comon,only:auxig,res6,sig_prob2
    
    IMPLICIT NONE

    double precision,intent(in)::frail,frail2,sig1!,sig2
    double precision,parameter::pi=3.141592653589793d0


    func2ASC = dexp(-res6(auxig)*dexp(frail+frail2)-(frail**2.d0)/(2.d0*sig1)&
            -(frail2**2.d0)/(2.d0*sig_prob2(auxig)))/(sqrt(2.d0*pi*sig1)*sqrt(2.d0*pi*sig_prob2(auxig)))
 
 !func2ASC = dexp(-res6(auxig)*dexp(sig1*frail+sig2*frail2)-(frail**2.d0)/(2.d0)&
    !        -(frail2**2.d0)/(2.d0))/2.d0*pi
 
    return

    end function func2ASC
