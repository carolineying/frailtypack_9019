...
!     end do
    ...
    do iii=1,ntimeAll
        do i=1,npred0
            ...
            if (predtimerec2(i,nrec0+1).gt.0) then 
                nreci(i) = nrec0
            end if 
    ! AK: I change the gap times to cumulated sums to find the interval from the time of the last recurrent event to the prediction time
            if (sum(predtimerec2(i,2:(nrec0+1))).lt.predTime) then
                predtimerec2(i,1) = predTime - sum(predtimerec2(i,2:(nrec0+1)))
            end if
...
!write(*,*)"predtimerec2",predTime2,survR(:,1)
            hazR(:,1) = lam(1)
                call survival_frailty(predTime,theR,theDC,nz+2,zi,surv,lam,nst)
    ...
    ! AK: Before it was the same for all the individuals (because we did not considered gaps but the time from the beginning to the prediction time)
                 predTime2 = predtimerec2(i,1)
!write(*,*)predTime2
        ...
        do i=1,npred0
            if (intcens.eq.0) then
                survRi = survR(i,:)
        !        write(*,*)i,nrec0,nreci(i),predtimerec2(i,:)
        !        write(*,*)"survRi",survRi,"survDC",survDC,"nrec",nreci
    ...
    !                write(*,*)'joint',i,ss11,ss12,ss11/ss12
                endif
