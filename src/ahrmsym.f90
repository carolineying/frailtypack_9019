...
!##########################################################################################
        ...
        subroutine fweight(weight,s,intrps,m,k,modofm,d)
...
!
        ...
        double precision::momnkn,weight
...
!
                ...
                do l = 0,nzrmax
                        ...
                        do i = 1,nzrmax
                                ...
                                !if(momprd(l,i).lt.0) then
                !               write(*,*)'a(i)',a(i),'momnkn',momnkn
                                !endif
                                end do
                end do
        end if
...
!
 ...
 10   intrps(1) = momprd(m(1),k(1))
        ...
        end subroutine fweight


        subroutine accumulate_permutation_count(s,m,limit,node_count)
        implicit none
        integer,intent(in)::s,limit
        integer,dimension(s),intent(in)::m
        integer,intent(inout)::node_count
        integer::i,groups,nonzero,remaining,count_group,choose_value
        integer,dimension(s)::counts

        ! A stable bounded multinomial count is simpler and less error-prone
        ! than reproducing FULSMH's permutation state machine.
        groups = 1
        counts(1) = 1
        do i=2,s
            if(m(i).eq.m(i-1))then
        ...
        remaining = 0
        do i=1,groups
            count_group = counts(i)
            call bounded_binomial(remaining+count_group,count_group,&
                max(0,limit/max(1,node_count)),choose_value)
            if(choose_value.gt.max(0,limit/max(1,node_count)))then
                node_count = limit+1
                return
            end if
            node_count = node_count*choose_value
            remaining = remaining+count_group
        end do
        nonzero = count(m.ne.0)
        do i=1,nonzero
            if(node_count.gt.limit/2)then
                node_count = limit+1
                return
            end if
            node_count = 2*node_count
        end do
        end subroutine accumulate_permutation_count


        subroutine bounded_binomial(n,k,limit,value)
        implicit none
        ...
        v = 1.d0
        do i=1,kk
            v = v*dble(n-kk+i)/dble(i)
            if(v.gt.dble(limit))then
                value = limit+1
                return
            end if
        end do
        value = nint(v)
        end subroutine bounded_binomial


        subroutine next_hrm_rule_count(s,n,minord,maxord,fulsms,limit,intcls)
        implicit none
        integer,intent(in)::s,n,minord,maxord,limit
        double precision,dimension(n+1,*)::fulsms
        integer,intent(out)::intcls
        integer,parameter::maxdim=1000,maxrdm=25
        integer::d,prtcnt,modofm,node_count
        integer,dimension(maxdim)::m,k
        double precision::fulwgt
        double precision,dimension(maxdim)::intrps
        ...
        do while(d.le.min(maxord,maxrdm))
            ...
            do while(modofm.le.d)
                call fweight(fulwgt,s,intrps,m,k,modofm,d)
                if(fulsms(n+1,prtcnt).eq.0.d0.and.fulwgt.ne.0.d0)then
                    call accumulate_permutation_count(s,m,max(0,limit-intcls),&
                        node_count)
                    if(node_count.gt.limit-intcls)then
                        intcls = limit+1
                        return
                    end if
                    intcls = intcls+node_count
                end if
                call nxpart(prtcnt,s,m,modofm)
            end do
            d = d+1
        end do
        end subroutine next_hrm_rule_count
...
!===================================================================
        ...
        use cubature_settings,only:cubature_last_rule,cubature_signed_sum_status,&
            cubature_cap_blocked,cubature_log_positive,cubature_log_negative,&
            cubature_cancellation_ratio
        use posterior_adaptive_settings,only:posterior_adaptive_active,&
            posterior_adaptive_fallback_active,posterior_callback_returns_log,&
            initialize_signed_gk
...
!   Local variables.
!
        integer::i,intcls,numsms,predicted_intcls
        ...
        if(posterior_adaptive_active.eq.1.or.&
            posterior_adaptive_fallback_active.eq.1)&
            posterior_callback_returns_log = 1
        if (ndim .le. 4) then
!
!     Call product Gauss-Hermite Rule
                ...
                do while (neval .le. maxpts .and. rule .le. maxrul .and. (ifail.gt.0 .or. neval.lt.minpts))
                        ...
                        if(posterior_adaptive_active.eq.1.or.&
                            posterior_adaptive_fallback_active.eq.1)then
                            call next_hrm_rule_count(ndim,nf,mnrule,rule,&
                                work(3*nf+1),maxpts-neval,predicted_intcls)
                            if(predicted_intcls.gt.maxpts-neval)then
...
!write(*,*)'neval=',neval,'rule',rule,'ifail',ifail
                ...
                cubature_last_rule = max(0,rule-1)
        end if
...
!
        ...
        use cubature_settings,only:cubature_signed_sum_status,&
            cubature_log_positive,cubature_log_negative,&
            cubature_cancellation_ratio
        use posterior_adaptive_settings,only:posterior_callback_returns_log,&
            gk_log_positive,gk_log_negative,logspace_add,signed_log_value
...
!***********************************************************************
        ...
        external::f
        integer::s,n,minord,maxord,numsms,d,i,modofm,intcls,prtcnt,sum_status
        integer,parameter::maxdim=1000,maxrdm=25
        double precision::fulwgt,log_contribution
                ...
                ! write(*,*)'fulwgt',x
                        ...
                        do i=1,n
                                ...
                                if(posterior_callback_returns_log.eq.1)then
                                    if(fulwgt.ne.0.d0)then
                                        log_contribution = dlog(abs(fulwgt))+&
                                            fulsms(i,prtcnt)
                                        if(fulwgt.gt.0.d0)then
                                            gk_log_positive(i) = logspace_add(&
                                                gk_log_positive(i),log_contribution)
                                        else
                                            gk_log_negative(i) = logspace_add(&
                                                gk_log_negative(i),log_contribution)
                                        end if
                                    end if
                                    call signed_log_value(gk_log_positive(i),&
                                        gk_log_negative(i),intval(i),sum_status)
                                    ! Early embedded rules can be nonpositive even
                                    ! when the final signed rule is valid.  Report
                                    ! the status of the current (hence final on
                                    ! return) approximation, not a sticky history.
                                    cubature_signed_sum_status = sum_status
                                    if(i.eq.1)then
                                        cubature_log_positive = gk_log_positive(i)
                                        cubature_log_negative = gk_log_negative(i)
                                        if(gk_log_negative(i).le.&
                                            -0.5d0*huge(1.d0))then
                                            cubature_cancellation_ratio = 0.d0
                                        else if(gk_log_positive(i).le.&
                                            -0.5d0*huge(1.d0))then
                                            cubature_cancellation_ratio = huge(1.d0)
                                        else
                                            cubature_cancellation_ratio = dexp(&
                                                min(0.d0,gk_log_negative(i)-&
                                                gk_log_positive(i)))
                                        end if
                                    end if
                                else
...
!
 ...
 20   sumcls = sumcls + 1
        ...
        do i = 1,n
                if(posterior_callback_returns_log.eq.1)then
                    fulsms(i) = logspace_add(fulsms(i),dlog(intwgt)+funval(i))
                else
...
!###########################################################################################
        ...
        use posterior_adaptive_settings,only:posterior_adaptive_active,&
            posterior_adaptive_fallback_active,logspace_add
...
!
        ...
        double precision::wtprod,log_term
        double precision,dimension(*)::point,weight,intval,x,funs,ic
        double precision,dimension(numfun)::log_integral
 ...
 10   wtprod = 1
        ...
        do i = 1,numfun
                ...
                if(posterior_adaptive_active.eq.1.or.&
                    posterior_adaptive_fallback_active.eq.1)then
                    log_term = dlog(wtprod)+funs(i)
                    log_integral(i) = logspace_add(log_integral(i),log_term)
                else
        ...
        if(posterior_adaptive_active.eq.1.or.&
            posterior_adaptive_fallback_active.eq.1)then
            do i=1,numfun
                if(log_integral(i).gt.dlog(huge(1.d0)))then
                    intval(i) = huge(1.d0)
                else
                    intval(i) = dexp(log_integral(i))
                end if
            end do
        end if
...
!###########################################################################"""
        ...
        use cubature_settings,only:cubature_last_rule,cubature_cap_blocked
        use posterior_adaptive_settings,only:posterior_adaptive_active,&
            posterior_adaptive_fallback_active
...
!
 ...
 10   if (neval + rule**ndim .le. maxpts .and. rule .lt. maxrul) then
        ...
        !       write(*,*)neval,rule,epsrel*abs(result(i)),abserr(i)
                ...
                cubature_last_rule = rule
                rule = rule + 1
                if (ifail .gt. 0 .or. neval .lt. minpts) goto 10
        end if
        if((posterior_adaptive_active.eq.1.or.&
            posterior_adaptive_fallback_active.eq.1).and.&
            ifail.gt.0.and.&
            neval+rule**ndim.gt.maxpts)cubature_cap_blocked = 1
!
