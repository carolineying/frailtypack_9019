prediction <- function(fit, data, data.Longi, t, window, event = "Both", conditional=FALSE, MC.sample=0, individual){
	...
	if (class(fit) == "jointPenal"){	
		...
		listPrec <- NULL 
	#	print(predtimerec)
	#	print(paste("predTime",predTime))
		if(!is.matrix(predtimerec)) predtimerec <- matrix(predtimerec, nrow = 1)
		# AK: If gap times I create and additional matrix with cumulated times (to know what was the history of recurrent events)
		if(fit$AG==FALSE){predtimerec2 <- t(apply(predtimerec, 1, cumsum))} #Dec-2017
	
		for (k in 1:nrow(predtimerec)){
		  # AK: We compare with cumulated time
			tPrec <- which(predtimerec2[k,] < predTime) #Dec-2017
			...
			taille = taille+1}else{ 
			  # As in the data we have also one row for terminal event per individual, we must take it into account in creating this variable for indexing covariates
			  taille = length(rowTimes)+taille+1 #30-11-2017
			}
		#	print(paste(k,"taille=",taille,"rowTimes",rowTimes,"tPrec",tPrec))
			listPrec <- c(listPrec,tPrec)                				
		}
		predtimerec <- replace(predtimerec, is.na(predtimerec),0) #30-11-16	
	# AK: I think we should also include this change, otherwise gap times are forwarded to Fortran code 
		# where the number of recurrent events (for the power of frailty in the formula) is the number of times smaller or equal than prediction time
#		predtimerec <- replace(predtimerec, predtimerec2>predTime,0) #Dec-2017 
	#	print(predtimerec)
#		print(predtimerec2)
