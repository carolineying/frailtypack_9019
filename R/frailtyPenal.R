"frailtyPenal" <-
  function (formula, formula.terminalEvent, data, recurrentAG=FALSE, cross.validation=FALSE, jointGeneral, n.knots, kappa,maxit=300, 
	...
	LIMparam=1e-3, LIMlogl=1e-3, LIMderiv=1e-3, print.times=TRUE, covMatrix1 = NULL, covMatrix2 = NULL, proband = NULL, 
	currentage = NULL, agemin = 0, sig.fixed = NULL, Fisher.info = FALSE,
	cubature.control = list(), integration.method = c("genz-keister",
	"adaptive-genz-keister"), integration.failure = c("fail", "floor"),
	adaptive.control = list()){
	...
	m$formula.terminalEvent <- m$n.knots <- m$recurrentAG <- m$cross.validation <- m$jointGeneral <- m$kappa <- m$maxit <- m$hazard <- m$nb.int <- m$RandDist <- m$betaorder <- m$betaknots <- m$init.B <- m$LIMparam <- m$LIMlogl <- m$LIMderiv <-  m$print.times <- m$init.Theta <- m$init.Alpha <- m$Alpha <- m$init.Ksi <- m$Ksi <- m$init.Eta <- m$Eta <- m$initialize <- m$covMatrix1 <- m$covMatrix2 <- m$agemin <- m$sig.fixed <- m$Fisher.info <- m$cubature.control <- m$integration.method <- m$integration.failure <- m$adaptive.control <- m$... <- NULL    
    ...
    correlated.fit <- !is.null(covMatrix1)
    if(!is.null(covMatrix1) && any(dim(covMatrix1) != dim(data)[1]))stop("The covariance matrix for random effects must be a square matrix of dimension corresponding to the number of individuals in the data")
     if(!is.null(covMatrix2) && any(dim(covMatrix2) != dim(data)[1]))stop("The covariance matrix for random effects must be a square matrix of dimension corresponding to the number of individuals in the data")
    if(!is.null(covMatrix2) && is.null(covMatrix1))stop("The covariance matrix covMatrix2 can be provided only if covMatrix1 is given as well.")
    cubature.defaults <- list(minpts = 30L, maxpts = 750L,
                              epsabs = 1e-100, epsrel = 1e-100)
    if (!is.list(cubature.control)) stop("cubature.control must be a named list.")
    unknown.cubature <- setdiff(names(cubature.control), names(cubature.defaults))
    if (length(unknown.cubature)) {
      stop("Unknown cubature.control value(s): ", paste(unknown.cubature, collapse = ", "))
    }
    cubature.control <- utils::modifyList(cubature.defaults, cubature.control)
    if (length(cubature.control$minpts) != 1L || !is.finite(cubature.control$minpts) ||
        cubature.control$minpts < 1 || cubature.control$minpts != as.integer(cubature.control$minpts)) {
      stop("cubature.control$minpts must be one positive integer.")
    }
    if (length(cubature.control$maxpts) != 1L || !is.finite(cubature.control$maxpts) ||
        cubature.control$maxpts < cubature.control$minpts ||
        cubature.control$maxpts != as.integer(cubature.control$maxpts)) {
      stop("cubature.control$maxpts must be an integer not smaller than minpts.")
    }
    for (nm in c("epsabs", "epsrel")) {
      value <- cubature.control[[nm]]
      if (length(value) != 1L || !is.finite(value) || value <= 0) {
        stop("cubature.control$", nm, " must be one finite positive number.")
      }
    }
    integration.method <- match.arg(integration.method)
    integration.failure <- match.arg(integration.failure)
	adaptive.defaults <- list(mode.tol = 1e-8, mode.maxit = 80L,
                              hessian.eps = 1e-10, fallback = TRUE,
                              diagnostics = FALSE, warm.start = TRUE)
    if (!is.list(adaptive.control)) stop("adaptive.control must be a named list.")
    unknown.adaptive <- setdiff(names(adaptive.control), names(adaptive.defaults))
    if (length(unknown.adaptive)) {
      stop("Unknown adaptive.control value(s): ",
           paste(unknown.adaptive, collapse = ", "))
    }
    adaptive.control <- utils::modifyList(adaptive.defaults, adaptive.control)
    if (length(adaptive.control$mode.tol) != 1L ||
        !is.finite(adaptive.control$mode.tol) || adaptive.control$mode.tol <= 0) {
      stop("adaptive.control$mode.tol must be one finite positive number.")
    }
    if (length(adaptive.control$mode.maxit) != 1L ||
        !is.finite(adaptive.control$mode.maxit) ||
        adaptive.control$mode.maxit < 1L ||
        adaptive.control$mode.maxit != as.integer(adaptive.control$mode.maxit)) {
      stop("adaptive.control$mode.maxit must be one positive integer.")
    }
    if (length(adaptive.control$hessian.eps) != 1L ||
        !is.finite(adaptive.control$hessian.eps) ||
        adaptive.control$hessian.eps <= 0 || adaptive.control$hessian.eps >= 1) {
      stop("adaptive.control$hessian.eps must be strictly between zero and one.")
    }
    for (nm in c("fallback", "diagnostics", "warm.start")) {
      if (length(adaptive.control[[nm]]) != 1L ||
          !is.logical(adaptive.control[[nm]]) || is.na(adaptive.control[[nm]])) {
        stop("adaptive.control$", nm, " must be TRUE or FALSE.")
      }
    }
    if (integration.method == "adaptive-genz-keister") {
      if (RandDist != "LogN" || is.null(covMatrix1)) {
        stop("adaptive-genz-keister is available only for correlated LogN frailty fits with covMatrix1.")
      }
      if (typeof != 2L) {
        stop("adaptive-genz-keister is currently implemented for the correlated Weibull likelihood only.")
      }
      if (!isTRUE(recurrentAG)) {
        stop("adaptive-genz-keister currently requires recurrentAG = TRUE, matching the implemented correlated family callback.")
      }
      if (cubature.control$maxpts > 7500L) {
        stop("adaptive-genz-keister is limited to at most 7,500 integrand evaluations per family.")
      }
    }
    if(!is.null(covMatrix2)){
      if(is.null(sig.fixed) || !is.numeric(sig.fixed) || length(sig.fixed) != 1L ||
      ...
      if(!missing(init.Theta))
        .prepare_two_covariance_init(init.Theta, sig.fixed)
    }
    ...
    if (length(cluster)){
		...
		ordre <- seq_len(nrow(m)) # ordre sera recalcule proprement plus bas
    ...
    # Robust row mapping after any sorting; do not rely on numeric row names.
    ordre <- match(row.names(m), row.names(m0))
    if (any(is.na(ordre))) stop("Internal row-mapping error while reordering data.")
    ordre.data <- ordre
    if (!missing(data) && !is.null(data)) {
        ordre.data <- match(row.names(m), row.names(data))
        if (any(is.na(ordre.data))) stop("Internal row-mapping error between model frame and input data.")
    }
...
#	}    
  ...
  #		ind.place <- ind.place -1
    ...
    if (!joint & !length(subcluster)){
		...
		sig_fixed <- 0
		if(is.null(covMatrix1)){covMatrix1 <- matrix(rep(0, n*n), nrow = n)
		covMatrix2 <- matrix(rep(0, n*n), nrow = n)
		kmat <- 0
		}else if(!is.null(covMatrix1) && is.null(covMatrix2)){kmat <- 1
		covMatrix2 <- matrix(rep(0, n*n), nrow = n)
		if(!is.null(sig.fixed)){sig_fixed <- sig.fixed
		np <- np - 1
		kmat <- 3}
		}else if(!is.null(covMatrix1) && !is.null(covMatrix2)){kmat <- 2
		np <- np  #+1
		sig_fixed <- sig.fixed
		#	Frailty <- 2
		}
		
		if(length(Fisher.info)>1){
		  if(length(Fisher.info)!= np)stop("Wrong number of parameter estimates in Fisher.info")
		  Fisher <- c(1, Fisher.info)
		}else{
		  Fisher <- c(0, rep(0,np))
		}
      ...
      # traitement de l'initialisation du Beta rentre par l'utilisateur
		...
		if (!missing(init.Theta)) {
		  if(kmat<2){
		    ...
		    if (Frailty==FALSE) stop("Frailty must be present in the correlated frailty model")
		    # With sig.fixed, covMatrix1 has a fixed variance and only the
		    # covMatrix2 variance is optimized. Previously two values were
		    # written here even though the optimizer vector has only one free
		    # variance slot; the first value overwrote a baseline-hazard start.
		    Beta[np-nvar] <- .prepare_two_covariance_init(init.Theta, sig.fixed)
		  }
		}
		...
		if(is.null(proband))proband <- rep(0, n)
		if(is.null(currentage))currentage <- rep(agemin, n)
		if (exists("ordre")) {
			data_n <- if (!missing(data) && !is.null(data)) nrow(data) else NA_integer_
			# Reorder only user-provided full-data structures; do not reorder n-by-n defaults.
			if (!is.na(data_n) && !is.null(covMatrix1) && nrow(covMatrix1) == data_n) covMatrix1 <- covMatrix1[ordre.data, ordre.data, drop = FALSE]
			if (!is.na(data_n) && !is.null(covMatrix2) && nrow(covMatrix2) == data_n) covMatrix2 <- covMatrix2[ordre.data, ordre.data, drop = FALSE]
			if (!is.na(data_n) && length(proband) == data_n) proband <- proband[ordre.data]
			if (!is.na(data_n) && length(currentage) == data_n) currentage <- currentage[ordre.data]
		}
		flush.console()
		if (print.times){
			ptm<-proc.time()
		...
		ans <- .Fortran(C_frailpenal,                      
                      ...
                      martingale.res=as.double(rep(0,as.integer(n))),#;length(uni.cluster)))),
                      martingaleCox=as.double(rep(0,n)),
                      frailty.pred=as.double(rep(0,as.integer(n))),#length(uni.cluster)))),
                      frailty.var=as.double(rep(0,as.integer(n))),#length(uni.cluster)))),
                      frailty.sd=as.double(rep(0,as.integer(n))),#length(uni.cluster)))),
                      ...
                      EPS=as.double(c(LIMparam,LIMlogl,LIMderiv)),
		                #Correlated frailty model 
		                Kmat1 = as.double(matrix(covMatrix1)),
		                Kmat2 = as.double(matrix(covMatrix2)),
		                corrRE_proband = as.integer(c(kmat, proband,
		                  cubature.control$minpts, cubature.control$maxpts)),
		                currentage = as.double(c(
		                  currentage,
		                  integration.method == "adaptive-genz-keister",
		                  adaptive.control$mode.maxit,
		                  adaptive.control$fallback,
		                  adaptive.control$diagnostics,
		                  adaptive.control$warm.start,
		                  integration.failure == "floor",
		                  adaptive.control$mode.tol,
		                  adaptive.control$hessian.eps,
		                  rep(0, length(uni.cluster) * (20L + 10L)))),
		                agemin_sig_fixed = as.double(c(agemin, sig_fixed,
		                  cubature.control$epsabs, cubature.control$epsrel)),
			                Fisher = as.double(Fisher),
			                I_fisher  = as.double(matrix(0,nrow=np,ncol=np))
						  )#,
      ...
      #AD:      
		...
		fit <- NULL
		fit$b <- ans$b
		fit$I_fisher <- matrix(ans$I_fisher, ncol = np)
      ...
      #AD:      
		...
		fit$Frailty <- Frailty
			if (Frailty){
				if (length(ans$frailty.pred) == n) {
					fit$martingale.res <- ans$martingale.res[order(ordre)]
					fit$frailty.pred <- ans$frailty.pred[order(ordre)]
				} else {
		...
		fit$EPS <- ans$EPS      
		fit$cubature.control <- cubature.control
		cubature.diagnostics <- ans$Kmat1[c(1L, 2L, n + 1L, n + 2L)]
		cubature.error <- ans$Kmat2[c(1L, 2L)]
		fit$cubature.diagnostics <- stats::setNames(as.integer(cubature.diagnostics),
		  c("calls", "failures", "max_neval", "max_dimension"))
			fit$cubature.error <- stats::setNames(cubature.error,
			  c("max_absolute", "max_relative"))
			fit$integration.method <- integration.method
			fit$integration.failure <- integration.failure
			fit$adaptive.control <- adaptive.control
			if (correlated.fit && logNormal == 1L) {
			  nfam <- length(uni.cluster)
			  adaptive.buffer <- ans$currentage[-seq_len(n + 8L)]
				  ni <- nfam * 20L
				  idiag <- matrix(as.integer(adaptive.buffer[seq_len(ni)]),
				                  nrow = nfam, ncol = 20L)
				  ddiag <- matrix(adaptive.buffer[ni + seq_len(nfam * 10L)],
				                  nrow = nfam, ncol = 10L)
			  method.labels <- c("genz-keister", "adaptive-genz-keister",
			                     "genz-keister-fallback",
			                     "adaptive-retained-rule",
			                     "adaptive-tempered-rescue",
			                     "adaptive-approximate-mode")
			  method.index <- pmin(pmax(idiag[, 3L], 0L), 5L) + 1L
			  family.ids <- unique(as.character(cluster))
			  if (length(family.ids) != nfam) family.ids <- seq_len(nfam)
			  family.integration <- data.frame(
			    family = family.ids,
			    dimension = idiag[, 2L],
			    method = method.labels[method.index],
			    mode.status = idiag[, 4L],
			    mode.iterations = idiag[, 5L],
			    gradient.norm = ddiag[, 1L],
			    hessian.condition = ddiag[, 2L],
			    hessian.min.eigenvalue = ddiag[, 3L],
			    hessian.min.stabilized = ddiag[, 4L],
			    hessian.regularized = as.logical(idiag[, 6L]),
			    rule = idiag[, 7L],
			    evaluations = idiag[, 8L],
			    error.estimate = ddiag[, 5L],
			    relative.error.estimate = ddiag[, 6L],
			    quadrature.status = idiag[, 9L],
			    log.integral = ddiag[, 7L],
				    fallback.used = as.logical(idiag[, 10L]),
				    reached.cap = as.logical(idiag[, 11L]),
				    quadrature.failed = as.logical(idiag[, 12L]),
					    covariance.factorization.status = idiag[, 13L],
					    signed.sum.status = idiag[, 14L],
					    floor.applied = as.logical(idiag[, 15L]),
					    floor.count = idiag[, 16L],
					    floor.signed.nonpositive.count = idiag[, 17L],
					    floor.nonpositive.value.count = idiag[, 18L],
					    floor.prior.under.threshold.count = idiag[, 19L],
					    floor.adaptive.under.threshold.count = idiag[, 20L],
					    signed.log.positive = ddiag[, 8L],
				    signed.log.negative = ddiag[, 9L],
				    signed.cancellation.ratio = ddiag[, 10L],
				    stringsAsFactors = FALSE)
			  ## In adaptive mode the only successful native status is exactly
			  ## one.  Treat impossible positive values as failures too; this also
			  ## prevents a corrupted diagnostic such as status 9 being silently
			  ## classified as convergence.
				  family.evaluated <- family.integration$dimension > 0L
				  mode.fail <- if (identical(integration.method,
				                              "adaptive-genz-keister")) {
				    family.evaluated & family.integration$mode.status != 1L
				  } else {
				    rep(FALSE, nrow(family.integration))
				  }
				  quad.fail <- family.evaluated &
				    family.integration$quadrature.failed
			  ## Native hard failures use +/-huge() sentinels.  Those values are
			  ## technically finite to R, so exclude the sentinel range explicitly.
				  finite.integral <- family.evaluated &
				    is.finite(family.integration$log.integral) &
				    abs(family.integration$log.integral) < .Machine$double.xmax / 2
			  ## HRMSYM status 1 means the requested tolerance could not be met
			  ## within maxpts.  The final completed rule can use fewer than maxpts,
			  ## so this classification must not require evaluations == maxpts.
				  budget.exhausted <- family.evaluated &
				    family.integration$quadrature.status == 1L & finite.integral
					  hard.failure <- family.evaluated &
					    !family.integration$floor.applied & (!finite.integral |
					    (family.integration$quadrature.status != 0L &
					      !budget.exhausted))
				  family.integration$evaluated <- family.evaluated
				  family.integration$finite.integral <- finite.integral
			  family.integration$budget.exhausted <- budget.exhausted
				  family.integration$hard.failure <- hard.failure
				  evals <- family.integration$evaluations
				  floor.uses <- sum(family.integration$floor.count)
				  families.ever.floored <- sum(family.integration$floor.count > 0L)
					  fit$integration.summary <- list(
				    families = nfam,
				    families.evaluated = sum(family.evaluated),
				    families.not.evaluated = sum(!family.evaluated),
			    mode.failures = sum(mode.fail),
			    mode.failure.percent = 100 * mean(mode.fail),
			    quadrature.failures = sum(quad.fail),
			    quadrature.failure.percent = 100 * mean(quad.fail),
			    reached.evaluation.cap = sum(family.integration$reached.cap),
			    budget.exhausted = sum(budget.exhausted),
				    fallback.families = sum(family.integration$fallback.used),
				    floor.policy = integration.failure,
				    floor.value = 1e-300,
				    floor.applications = floor.uses,
				    families.ever.floored = families.ever.floored,
				    families.floored.at.final.evaluation =
				      sum(family.integration$floor.applied),
				    floor.reason.counts = c(
				      signed.nonpositive =
				        sum(family.integration$floor.signed.nonpositive.count),
				      nonpositive.value =
				        sum(family.integration$floor.nonpositive.value.count),
				      prior.under.threshold =
				        sum(family.integration$floor.prior.under.threshold.count),
				      adaptive.under.threshold =
				        sum(family.integration$floor.adaptive.under.threshold.count)),
				    hard.failures = sum(hard.failure),
				    nonfinite.integrals =
				      sum(family.evaluated & !finite.integral),
				    covariance.factorization.failures =
				      sum(family.integration$covariance.factorization.status != 0L),
				    hessian.regularizations = sum(family.integration$hessian.regularized),
			    maximum.dimension = max(family.integration$dimension),
			    evaluation.quantiles = stats::quantile(evals,
			      probs = c(0, .25, .5, .75, 1), names = TRUE, type = 1))
			  fit$integration.converged <- !any(mode.fail | quad.fail)
				  fit$integration.valid <- !any(hard.failure) && floor.uses == 0L
				  fit$adaptive.integration.completed <-
				    !any(mode.fail | family.integration$fallback.used |
				      hard.failure) && floor.uses == 0L
				  floor.columns <- c(
				    "family", "dimension", "floor.applied", "floor.count",
				    "floor.signed.nonpositive.count",
				    "floor.nonpositive.value.count",
				    "floor.prior.under.threshold.count",
				    "floor.adaptive.under.threshold.count")
				  fit$integration.floor <- list(
				    policy = integration.failure,
				    value = 1e-300,
				    applications = floor.uses,
				    families = families.ever.floored,
				    by.family = family.integration[
				      family.integration$floor.count > 0L, floor.columns,
				      drop = FALSE])
				  fit$integration.status <- if (any(hard.failure)) {
				    3L
				  } else if (floor.uses > 0L) {
				    4L
				  } else if (any(family.integration$fallback.used)) {
			    2L
			  } else if (any(budget.exhausted)) {
			    1L
			  } else {
			    0L
			  }
				  fit$integration.status.label <- switch(
				    as.character(fit$integration.status),
				    `0` = "tolerance-met",
				    `1` = "budget-exhausted",
				    `2` = "fallback-used",
				    `3` = "hard-failure",
				    `4` = "floor-used")
				  if (isTRUE(adaptive.control$diagnostics)) {
				    fit$family.integration.diagnostics <- family.integration
				  }
				  if (floor.uses > 0L) {
				    warning(sprintf(
				      paste0("The 1e-300 integration floor was applied %d time(s) ",
				             "across %d family/families. Inspect ",
				             "fit$integration.floor and fit$integration.summary."),
				      floor.uses, families.ever.floored), call. = FALSE)
				  }
			}
      #
   ...
   #       		stop("Interaction terms are not valid for terminal event formula")
	...
	if (length(subcluster) & joint){			
		...
		ans <- .Fortran(C_joint, 
				...
				linear.pred = as.double(rep(0,n)),#as.double(matrix(0,nrow = n, ncol = 2)),#temp linear.pred and martingales for observations
	...
	#	fit$formula <- formula(Terms)
      ...
      # verif que les martingales ont ete bien calculees
		...
		#	}
		#}    
#		linear.pred <- matrix(ans$linear.pred, ncol = 2)
		
		fit$martingale.res <- MartinGale[,1] # seulement pour les familleslinear.pred[,2] #
		fit$martingaledeath.res <- MartinGale[,2]		
		fit$frailty.pred <- MartinGale[,3]   
